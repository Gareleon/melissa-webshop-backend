...
const Soap = require("../soaps/soap.model");
...
router.get("/", async (req, res) => {
  try {
    ...
    // 4. Trending soaps statistics:
    const trendingSoapsCount = await Soap.aggregate([
      { $match: { trending: true } }, // Match only trending books
      { $count: "trendingSoapsCount" }, // Return the count of trending books
    ]);

    // If you want just the count as a number, you can extract it like this:
    const trendingSoaps =
      trendingSoapsCount.length > 0
        ? trendingSoapsCount[0].trendingSoapsCount
        : 0;

    // 5. Total number of soaps
    const totalSoaps = await Soap.countDocuments();
